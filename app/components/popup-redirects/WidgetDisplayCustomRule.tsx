...
export default function WidgetDisplayCustomRule({ appId, setToastData }) {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const responseEditor = await fetch(GET_WIDGET_EDITOR);
      ...
      const responseEditorStatus = await fetch(GET_WIDGET_EDITOR_STATUS);
  ...
  async function handleCustomCodeStatus(value, appId) {
    ...
    if (appId) {
      ...
      try {
        const response = await fetch(UPDATE_WIDGET_EDITOR_STATUS, {
          headers: {
  ...
  async function handleCustomCodeSave() {
    ...
    if (data !== "") {
      try {
        const response = await fetch(UPDATE_WIDGET_EDITOR, {
          headers: {
            "Content-Type": "application/json",
          },
          method: "post",
