...
import CustomizeMarketsPopup from "app/components/markets-popup/CustomizeMarketsPopup";
...
export default function CustomizePopupPage() {
  ...
  useEffect(() => {
    ...
    const definedAdvancedConfigs = { ...default_advanced_configs, ...marketsConfigs?.data?.advancedConfigs };
  ...
  return (
    ...
    >
      ...
      <SaveBar id="configs-save-bar" discardConfirmation>
        <button variant="primary" onClick={saveConfigs} loading={loading[ACTIONS.update_MarketsConfigs + "Loading"] ? "true" : undefined}></button>
        <button onClick={() => {
          shopify.saveBar.hide('configs-save-bar');
